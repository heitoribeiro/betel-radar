...
import {listActiveListings,listVerifiedLocationReferences,patchListing} from './supabase-rest.js';
...
function simpleNeighborhood(v=''){return String(v).replace(/\s*\([^)]*\)\s*/g,' ').replace(/\s+/g,' ').trim()}
...
function referenceMatch(row){
  if(!row.neighborhood||!row.city)return null;
  const wanted=norm(row.neighborhood),simple=norm(simpleNeighborhood(row.neighborhood)),city=norm(row.city),state=norm(row.state||'BA');
  return verifiedReferences.find(ref=>{
    if(norm(ref.city)!==city||norm(ref.state||'BA')!==state)return false;
    const names=[ref.normalized_name,ref.name,...(Array.isArray(ref.aliases)?ref.aliases:[])].filter(Boolean).map(norm);
    return names.some(v=>v===wanted||v===simple)
  })||null
}
...
function scoreNeighborhood(result,row,ctx){
  ...
  const wanted=row.neighborhood;const simplified=simpleNeighborhood(wanted);
  if(!localityMatch(result,wanted)&&(!simplified||!localityMatch(result,simplified)))return -Infinity;
  const inCity=cityMatch(result,row)||insideBounds(result,ctx?.bounds);if(!inCity)return -Infinity;
  const a=result.address||{};const wantedNorm=norm(wanted),simpleNorm=norm(simplified);let score=0;
  const rn=norm(result.name||'');if(rn===wantedNorm||rn===simpleNorm)score+=8;
  if([a.neighbourhood,a.suburb,a.quarter,a.city_district,a.hamlet,a.village].filter(Boolean).map(norm).some(v=>v===wantedNorm||v===simpleNorm))score+=7;
...
function scoreAddress(result,row,ctx){
  ...
  if(row.neighborhood&&(localityMatch(result,row.neighborhood)||localityMatch(result,simpleNeighborhood(row.neighborhood))))score+=5;
...
function queriesForOption(option,row){
  if(option.precision!=='neighborhood')return [option.query];
  const state=row.state||'BA';const city=row.city||'';const raw=row.neighborhood||'';const simple=simpleNeighborhood(raw);
  const qs=[option.query];
  if(simple&&norm(simple)!==norm(raw))qs.push([simple,city,state,'Brasil'].filter(Boolean).join(', '));
  qs.push([raw,state,'Brasil'].filter(Boolean).join(', '));
  if(simple&&norm(simple)!==norm(raw))qs.push([simple,state,'Brasil'].filter(Boolean).join(', '));
...
async function resolveRow(row){
  ...
  const reference=referenceMatch(row);
  if(reference){
    return {action:'update',patch:{
      latitude:Number(reference.latitude),longitude:Number(reference.longitude),geocode_status:'resolved',
      geocode_precision:reference.precision||'neighborhood',geocode_source:'reference_catalog',
      geocode_query:[row.neighborhood,row.city,row.state||'BA','Brasil'].filter(Boolean).join(', '),
      geocode_label:`${reference.name}, ${reference.city}, ${reference.state} · referência verificada (${reference.source})`,
      geocode_confidence:1,geocoded_at:now.toISOString(),location_signature:sig
    }}
  }

  const options=candidates(row);const ctx=await cityContext(row);
  if(!options.length){
    return {action:'update',patch:{latitude:null,longitude:null,geocode_status:'unresolved',geocode_source:'nominatim_v3',geocode_query:null,geocode_label:null,geocode_confidence:null,geocode_precision:null,geocoded_at:now.toISOString(),location_signature:sig}}
  }
  for(const option of options){
    for(const query of queriesForOption(option,row)){
      const results=await requestNominatim(query,10);const result=pickBest(results,option,row,ctx);if(!result)continue;
      const lat=Number(result.lat),lng=Number(result.lon);if(!Number.isFinite(lat)||!Number.isFinite(lng))continue;
      return {action:'update',patch:{
        latitude:lat,longitude:lng,geocode_status:'resolved',geocode_precision:option.precision,geocode_source:'nominatim_v3',geocode_query:query,
        geocode_label:result.display_name||query,geocode_confidence:Number.isFinite(Number(result.importance))?Number(result.importance):null,
        geocoded_at:now.toISOString(),location_signature:sig
      }}
    }
  }
  return {action:'update',patch:{latitude:null,longitude:null,geocode_status:'unresolved',geocode_source:'nominatim_v3',geocode_query:options[0]?.query||null,geocode_label:null,geocode_confidence:null,geocode_precision:null,geocoded_at:now.toISOString(),location_signature:sig}}
}

async function main(){
  const rows=await listActiveListings();verifiedReferences=await listVerifiedLocationReferences();const max=Math.max(1,Number(env('BETEL_GEOCODE_MAX_ROWS','50'))||50);
  const stats={active:rows.length,references:verifiedReferences.length,checked:0,resolved:0,unresolved:0,skipped:0,errors:0,updated:0};
  ...
  console.log(JSON.stringify({ok:stats.errors===0,provider:'nominatim_v3+reference_catalog',algorithm:ALGO_VERSION,cache_queries:queryCache.size,stats},null,2));
