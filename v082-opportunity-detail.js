/* Betel Radar v0.8.2 — ficha de origem e verificação, build 8208 */
(function(){
  const BUILD='8208';
  let currentId=null;
  let wrapTimer=null;
  ...
  function money(v){const n=Number(v);return Number.isFinite(n)?n.toLocaleString('pt-BR',{style:'currency',currency:'BRL',maximumFractionDigits:0}):'Não informado'}
  ...
  function installStyles(){
    ...
    const s=document.createElement('style');s.id='v082OpportunityDetailStyles';s.textContent=`
      .v082-origin-panel{margin:16px 0 4px;padding:14px;border:1px solid #e6e1da;border-radius:16px;background:linear-gradient(180deg,#fff 0%,#fbfaf8 100%);box-shadow:0 7px 20px rgba(20,20,20,.04);font-family:inherit;color:#171717}
      ...
      .v082-origin-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin-top:12px}.v082-origin-cell{border:1px solid #ece8e2;border-radius:12px;background:#fff;padding:9px 10px;min-width:0}.v082-origin-cell small{display:block;font-size:9px;color:#8b8b8b;margin-bottom:3px}.v082-origin-cell b{display:block;font-size:11px;line-height:1.35;overflow-wrap:anywhere}
      ...
      @media(max-width:760px){.v082-origin-panel{padding:12px;border-radius:14px;margin-top:12px}.v082-origin-head{display:block}.v082-origin-badges{justify-content:flex-start;margin-top:8px}.v082-origin-grid,.v082-origin-check ul{grid-template-columns:1fr}.v082-origin-actions{display:grid;grid-template-columns:1fr}.v082-origin-btn{text-align:center;width:100%;box-sizing:border-box;padding:10px}.v082-origin-title{font-size:13px}.v082-origin-sub{font-size:10px}}
    `;document.head.appendChild(s)
  }

  function detailHost(op){
    const preferred=['#detailModal','#opportunityDetail','.detail-modal','.detail-panel','.opportunity-detail','[role="dialog"]','.modal.show','.modal.open','.drawer.open','.sheet.open'];
    for(const sel of preferred){for(const el of document.querySelectorAll(sel)){if(visible(el)&&(el.textContent||'').includes(op.title||''))return el}}
    ...
    for(const node of nodes){
      ...
      while(el&&el!==document.body){
        ...
        const modalish=/detail|modal|drawer|sheet|dialog|overlay|panel/i.test(sig)||['fixed','absolute'].includes(cs.position);
        if(modalish&&r.width>=Math.min(300,innerWidth*.72)&&r.height>=220)return el;
        el=el.parentElement
      }
    }
    return null
  ...
  function checklist(op){
    ...
    const priceOk=Number.isFinite(Number(op.price));
    const areaOk=Number.isFinite(Number(op.areaM2))&&Number(op.areaM2)>0;
  ...
  function scheduleInject(id=currentId){[30,120,320,700].forEach(ms=>setTimeout(()=>inject(id),ms))}
  ...
  function ensureWrap(){if(wrapTimer)clearTimeout(wrapTimer);wrapOpenDetail();wrapTimer=setTimeout(wrapOpenDetail,500)}
  ...
  document.addEventListener('click',e=>{
    ...
    setTimeout(neutralizeUnverifiedVisuals,100)
  },true);
  window.addEventListener('betel:real-data-ready',()=>{ensureWrap();setTimeout(neutralizeUnverifiedVisuals,120)});
  window.addEventListener('pageshow',()=>{ensureWrap();setTimeout(neutralizeUnverifiedVisuals,350)});
  new MutationObserver(ms=>{if(ms.every(m=>m.target?.closest?.('#v082SourceDetailPanel')))return;ensureWrap();if(currentId)setTimeout(()=>inject(currentId),80);setTimeout(neutralizeUnverifiedVisuals,120)}).observe(document.documentElement,{subtree:true,childList:true});
  installStyles();ensureWrap();setTimeout(neutralizeUnverifiedVisuals,900);
